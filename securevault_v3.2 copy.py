...
class Blockchain:
    ...
    def get_credential_by_attribute(self, attribute, public_key_hex):
        ...
        for block in self.chain:
            ...
            # Skip genesis block (string data) and non-credential blocks
            if not isinstance(data, dict) or data.get("type") != "identity_credentials":
                continue
    ...
    def _is_credential_revoked(self, attribute):
        ...
        for block in self.chain:
            ...
            # Skip genesis block (string data)
            if not isinstance(data, dict):
                continue
                
            if data.get("type") == "revocation":
                if data.get("revoked_attribute") == attribute:
                    return True
        return False
    ...
    def print_entire_blockchain(self, mykad_data=None):
        ...
        for i, block in enumerate(self.chain):
            print(f"\nBLOCK {block['index']}  ({'GENESIS' if i == 0 else self._get_block_type(block)})")
            print("─" * 80)
            ...
            elif isinstance(data, dict) and data.get("type") == "revocation":
                print(f"REVOCATION      : Attribute '{data['revoked_attribute']}' revoked")
            elif isinstance(data, dict) and data.get("type") == "identity_credentials":
    ...
    def _get_block_type(self, block):
        """Helper method to determine block type for display"""
        data = block["data"]
        if isinstance(data, dict):
            if data.get("type") == "identity_credentials":
        ...
        if choice == "1":
            ...
            if selected:
                ...
                for block in vault.chain:
                    data = block["data"]
                    if isinstance(data, dict) and data.get("type") == "identity_credentials":
                ...
                if identity_block:
                    ...
                    import copy
                    original_chain = copy.deepcopy(vault.chain)
                    ...
                    # Recompute the block hash after tampering
                    identity_block["hash"] = vault._compute_hash(identity_block)
